...
Run the section of setup code above first by clicking the green arrow in the upper right of the grey box. We'll mainly rely on the popular `dplyr` package. Packages are a common aspect of coding in R: they contain user-written functions designed to make analysis faster and easier.
...
Note that when we aren't using `<-` to assign the results of a line of code to an object in memory, R will simply produce the results in the console. 
...
First, let's rename some of these ugly variable (column) names with the `rename` function. If you're working with your own data, find one or more that you can change, even if just temporarily.
```{r}
...
See how useful our initial exercises with indexing have become? Any time we have a subset of positions generated by functions like `grep`, all we need to do is break out the brackets and commas.
...
#note to Alex: index grep call when knitting to pdf
```
