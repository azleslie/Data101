...
Welcome to Data 101! In the humanities and social sciences we often find ourselves with data - that is, a set of information that can be organized into consistent categories and that we want to understand as a whole. Sometimes that data is given to us and other times we assemble it ourselves; but in either case the data is only the beginning. In this workshop you'll learn how to carry out some of the essential operations for organizing, transforming, and analyzing your data.

We'll mainly rely on the popular `dplyr` package. Packages are a common aspect of coding in R: they contain user-written functions designed to make analysis faster and easier.

If you have a spreadsheet of your own that you'd like to work with, I encourage you to do so; just don't forget to change the names of objects and variables in my code accordingly. Otherwise, we'll be working with library transaction data from the Muncie, Indiana Public Library between 1892 and 1904, available thanks to the [What Middletown Read Project](https://lib.bsu.edu/wmr/).
...
First, let's read in our data into our environment. We'll do that by using a function, `read.csv`, to read in a .csv file and assign that data (using `<-`) to an object with whatever name we'd like ("transactions"). (If you want to try working with your own spreadsheets, change the object names and file names as desired.)
```{r}
transactions <- read.csv("transaction.csv", skip=1)
books <- read.csv("book.csv", skip=1)
```

Sometimes it is necessary to join multiple spreadsheets together in order to link related sets of data. In this case, we have two .csv files pertaining to the same data: the books held in the Muncie, Indiana public library from 1891 to 1902 and the patron transactions from the same library during the same years. Before we get started, we'll use `inner_join` to join our transactions data frame and our books data frame by each book's Accession Number. By default, `inner_join` never removes observations: wherever there is a shared value in the specified variable, it will combine all other variables from each of the joining data frames into a single row. Wherever there isn't a shared value (e.g., a book that was never checked out), `inner_join` will simply leave the values for any missing variables blank.
...
Our data frame includes variables of numerical data (such as Transaction Date and Accession number), as well as categorical (such as Title or Borrower Name). It also currently includes a variable or two that seem to contain a bit of both (such as Author).

R is handy because it allows us to select, inspect, and mutate our data in a wide range of possible ways. We can select values from a data frame by indicating their specific position within it. For example, let's say we wanted to know the name of the first book checked out. We know that the first transaction is the first observation in our data frame because our observations are arranged chronologically by transaction date; we also know Title is the third variable in our data frame. So all we need to do is index that exact position, row one column one, into all_records.
```{r}
all_records[1,3]
```

If we wanted to see all values belonging to that observation, we would simply index the number of the row followed by a comma, without a second number.
```{r}
all_records[1,]
```

If we instead wanted to see all values belonging to that variable, we could in similar fashion index the number of the column preceded by a comma without indexing the row. I've then gone and indexed the results again so as to only return the first ten - otherwise there would be 173,515!
```{r}
all_records[,3][1:10]
```

But since our variables have names, we can achieve the same result by using the `$` operator followed by the name of the desired variable; the results are exactly the same.
```{r}
all_records$Title.x[1:10]
```

## Basic Organizational Operations

The `dplyr` package gives us seven basic functions for organizing and analyzing data frames.

Function    | Use
------------|---------------
`rename`    | Rename a variable (column), new title = old title
`select`    | Select variables (columns) to include / exclude (with a negative operator)
`arrange`   | Arrange the order of observations, adding `desc` for descending order
`filter`    | Filter observations (rows) based on their values for a specified variable
...
`mutate`    | Create a new variable in a data frame by mutating an existing variable

We'll also be using the piping operator, `%>%`, which allows us to pipe the output of one function directly into the next: this allows us to make our code clearer and more concise. Think of it like pouring our data through a series of sifters with increasingly smaller withes.

First, let's rename some of these ugly variable (column) names with the `rename` function.
```{r}
...
All the piping operator does in those previous lines is tell the next function that we're still working with the all_records data frame. If we wanted to, we could achieve the exact same result by writing out each `rename` call separately: even with only two operations you can see the added redundancy.
```{r, eval=FALSE}
all_records <- rename(all_records, "Accession" = "Accession..")
all_records <- rename(all_records, "Title" = "Title.x")
```

Our data frame has two variables for titles that each contain the exact information as a result of our `inner_join`. We can remove the duplicate and any other variables we don't want by using a negative operator  `select` 
```{r}
all_records <- all_records %>%
  select(-Title.y) %>%
  select(-Patron..) %>%
...
We can also systematically transform, or mutate, values within the data frame. For this we'll use `mutate`, a function that will allow us to declare a new variable within the data frame based on an existing variable. As with `rename`, the new variable comes first and the existing one comes second. You may have noticed that our dates are currently a bit of an eyesore: let's mutate all the dates at once into a format that's more readable for both us and R.
```{r}
all_records <- all_records %>%
...
Since this mutation is just reformating rather than creating new data, we can remove the old variable. Let's go ahead and make that same mutation for the Accession Date, i.e., the date when the book was added to the library catalog.
```{r}
all_records <- all_records %>%
...
Now that these dates are properly formatted, we can use `mutate` to generate new information. How old was each book when it was checked out? Let's run a quick test by indexing only the first value in each column:
```{r}
...
`select` is only for selecting variables/columns. When we want to identify observations/rows, however, we `filter`. `filter` is particularly handy when working with numeric (or pseudo-numeric) data. Here, for example, we can filter out all observations with transaction dates earlier than (less than) 1893.
```{r}
...
```{r, include=FALSE}
rm(transactions, books, records_before_1893, records_by_borrower)
...
Second, the only variables retained in any operation ending with `summarize` are the ones explicitly specified for summary (in this case, Author and Total). We might, however, want to have a slightly less sweeping summary, such as one that groups observations first by Author and then by Text.
```{r}
...
There's a lot of information here. If we indeed wanted this data frame to include only observations in which the same person checked out the same book more than once, what would we have to add? If we then wanted to sort our data by the number of checkouts, what would we have to add?[^4]
...
Let's say we wanted to find all the observations (rows) that have a particular value for a particular variable (column): in this case, the number of books checked out by a single author. We can use filter so long as we have the exact name of the desired Author, for example:
```{r, max.print=10, rows.print=10}
all_records %>%
  filter(Author == "Howells, William Dean, 1837-1920.")
```

But what if some of our observations record Howells' lifespan differently - as, in fact, some of them do? What we want is a search that can locate just part of the value. To do that, we'll use an R function, `grep`, that searches for an exact character pattern within a vector. (I'll index the results so that we only see the first ten.)
```{r}
grep("Howells, William", all_records$Author)[1:10]
```

This gives us a long list of numbers. What are these numbers? They're the position of each hit in `test$Author`; in other words, they correspond to the observation or row in our data frame. What we actually want to see is all the information pertaining to each observation: to do so, we'll index those positions into the data frame (using a comma followed by nothing in order to get the entire row for each hit):
```{r, max.print=10, rows.print=10}
all_records[grep("Howells, William", all_records$Author),]
```

We can make our searches even more particular. For example, we could just as easily carry out the same operation but for all books that contain the word "adventure" in their titles. (Because `grep` is only concerned with matching the exact character pattern and nothing outside the quotation marks, it will return values that include "adventures" or "adventured" but not "adventuring").
```{r, max.print=10, rows.print=10}
...
Since we're looking to create a new variable based on an existing one, we're definitely going to want to use `mutate`. How exactly are we looking to `mutate`? Let's look at the values for the Author variable again more closely.

We know each entry begins with a name composed of characters, but the length of that name varies (and sometimes includes a nom de plume in parentheses). We know that this followed by a four-digit number representing year of birth, but there is some variance in what separates them: sometimes just a comma, sometimes a comma and a symbol. Year of birth, however, is always followed by a dash, followed by a four-digit number representing year of death. Sometimes year of death is followed by a period, but not always.
...
Another great benefit of working with data in R is that one can easily transition from analysis to visualization. For example, we can generate a quick graph of the top_authors data frame we made earlier:
```{r}
top_authors[2:11,] %>%
...
This graph is nice in itself. But even more importantly, it makes obvious something that we might've otherwise overlooked: the fact that the number of transactions follows a seasonal rhythm.
...
[^2]:Solution:
    ...
    ```{r, eval=FALSE}
    round(as.numeric(all_records$Transaction.Date - all_records$Accession.Date)/365.25, digits=2)
    ```
