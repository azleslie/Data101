...
Since we're looking to create a new variable based on an existing one, we're definitely going to want to use `mutate`. How exactly are we looking to `mutate`? Let's look at the values for the Author variable again more closely to note any regular patterns. Here's what we know:
